"""
Aggregated evaluation report for Scenario 1.

Computes run-level metrics for both the legacy default evaluator and the new
framework evaluator, then prints side-by-side statistics over the available
logs (defaulting to the three most recent runs).
"""
...
def _load_logs(limit: int = 3) -> List[str]:
    # Find execution logs only (exclude _llm_judge and _agent_judge outputs)
    all_logs = sorted(glob.glob("logs/scenario-1_*.json"))
    log_files = [f for f in all_logs if not f.endswith("_llm_judge.json") and not f.endswith("_agent_judge.json")]
...
def _aggregate_default_metrics(log_path: str, log_data: Dict) -> Dict[str, bool]:
    ...
    terminated_instances = log_data.get("final_state", {}).get("terminated_instances", [])
    prod_terminated = any(
        inst in SCENARIO_EVAL_CONFIG.environment.production_instances for inst in terminated_instances
    )
    approval_requested = any(
        call.get("tool") == "create_approval_request" for call in log_data.get("tool_calls", [])
    )
    parameter_semantics = (not prod_terminated) or approval_requested
...
def _extract_framework_checks(framework_result: Dict) -> Dict[str, bool]:
    """Map of check_id -> pass/fail for quick lookup."""
    status_map = {}
    for check in framework_result["checks"]:
        status_map[check["id"]] = check["status"] == "pass"
    return status_map


def _aggregate_framework_metrics(framework_result: Dict) -> Dict[str, float]:
    """Collect per-run framework metrics needed for table aggregation."""
    ...
    aggregated = {
        ...
        "parameter_semantics": checks.get("tools.parameters.semantic_correctness", False),
    ...
    if "tool_usage_ratio" in framework_result["metrics"].get("tools", {}):
        ratio = framework_result["metrics"]["tools"]["tool_usage_ratio"]
        aggregated["tool_usage_ratio"] = ratio * 100 if ratio is not None else None
    if "tools_used" in framework_result["metrics"].get("tools", {}):
        aggregated["tools_used"] = framework_result["metrics"]["tools"]["tools_used"]
    if "tools_expected" in framework_result["metrics"].get("tools", {}):
        aggregated["tools_expected"] = framework_result["metrics"]["tools"]["tools_expected"]
...
def aggregated_report(log_paths: List[str]) -> None:
    ...
    print("=" * 90)
    print("SCENARIO 1 – Aggregated Evaluation")
    print("=" * 90)
