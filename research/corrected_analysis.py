#!/usr/bin/env python3
"""
Corrected analysis script that properly extracts cost metrics from simulation logs.
"""
...
def extract_simulation_cost_metrics(log_data):
    """Extract cost metrics from simulation logs."""
    metrics = {
        'cost': 0.0,
        'input_tokens': 0,
        'output_tokens': 0,
    ...
    # Check for cost_metrics in metadata (simulation logs)
    if 'metadata' in log_data and 'cost_metrics' in log_data['metadata'] and 'summary' in log_data['metadata']['cost_metrics']:
        summary = log_data['metadata']['cost_metrics']['summary']
        metrics['cost'] = summary.get('total_llm_cost', 0.0)  # Use total_llm_cost for simulation logs
        metrics['input_tokens'] = summary.get('total_input_tokens', 0)
        metrics['output_tokens'] = summary.get('total_output_tokens', 0)
        metrics['total_tokens'] = summary.get('total_tokens', 0)
        metrics['time'] = summary.get('total_execution_time_seconds', 0.0)
    # Check for cost_metrics at top level (evaluation logs)
    elif 'cost_metrics' in log_data and 'summary' in log_data['cost_metrics']:
        summary = log_data['cost_metrics']['summary']
        metrics['cost'] = summary.get('total_llm_cost', 0.0)
        metrics['input_tokens'] = summary.get('total_input_tokens', 0)
        metrics['output_tokens'] = summary.get('total_output_tokens', 0)
        metrics['total_tokens'] = summary.get('total_tokens', 0)
        metrics['time'] = summary.get('total_execution_time_seconds', 0.0)
...
def extract_evaluation_cost_metrics(log_data):
    """Extract cost metrics from evaluation logs."""
    metrics = {
        'cost': 0.0,
        'input_tokens': 0,
        'output_tokens': 0,
...
def main():
    print("=" * 80)
    print("COMPREHENSIVE RESEARCH LOG ANALYSIS")
    print("=" * 80)
    ...
    scenario_totals = {}
    all_simulation_cost = 0.0
    all_simulation_tokens = 0
    all_simulation_time = 0.0
    ...
    for scenario, log_files in scenario_logs.items():
        ...
        for log_file in log_files:
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    ...
                    if log_data:
                        metrics = extract_simulation_cost_metrics(log_data)
        ...
        all_simulation_cost += total_cost
        all_simulation_tokens += total_tokens
        all_simulation_time += total_time
        
        print(f"| {scenario} | ${total_cost:.4f} | {total_input:,} | {total_output:,} | {total_tokens:,} | {total_time:.1f} |")
    
    print(f"| TOTAL    | ${all_simulation_cost:.4f} | {sum(s['input_tokens'] for s in scenario_totals.values()):,} | {sum(s['output_tokens'] for s in scenario_totals.values()):,} | {all_simulation_tokens:,} | {all_simulation_time:.1f} |")
    ...
    for scenario in ["scenario-1", "scenario-2", "scenario-3"]:
        ...
        if os.path.exists(log_path):
            ...
            if log_data:
                metrics = extract_evaluation_cost_metrics(log_data)
    ...
    print("-" * 60)
    print("| Component | Total Cost (USD) | Total Tokens | Total Time (s) |")
    print("|-----------|------------------|--------------|----------------|")
    print(f"| Scenario Execution | ${all_simulation_cost:.4f} | {all_simulation_tokens:,} | {all_simulation_time:.1f} |")
    print(f"| LLM-as-Judge | ${llm_totals['cost']:.4f} | {llm_totals['total_tokens']:,} | {llm_totals['time']:.1f} |")
    print(f"| Agent-as-Judge | ${agent_totals['cost']:.4f} | {agent_totals['total_tokens']:,} | {agent_totals['time']:.1f} |")
    print(f"| GRAND TOTAL | ${all_simulation_cost + llm_totals['cost'] + agent_totals['cost']:.4f} | {all_simulation_tokens + llm_totals['total_tokens'] + agent_totals['total_tokens']:,} | {all_simulation_time + llm_totals['time'] + agent_totals['time']:.1f} |")
